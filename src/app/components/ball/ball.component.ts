...
export class BallComponent implements OnChanges, OnInit, AfterViewInit {
  @Input() id!: number;
  ...
  handleBallEnter(e: KeyboardEvent) {
    if (this.startFlag && e.code === 'Enter') {
      this.playGame();
    }
  }
  ...
  playGame() {
    ...
    const {x: ballX, y: ballY} = this.ball.nativeElement.getBoundingClientRect();
  ...
  ngOnChanges(): void {
    ...
    if (!this.startFlag) {
      this.renderer.setStyle(this.ball.nativeElement, 'transform', `translateX(-50%)`);
      this.renderer.addClass(this.ball.nativeElement, 'center');
    } else {
      this.renderer.removeClass(this.ball.nativeElement, 'center');
    }
  ...
  moveBall(): void {
    ...
    this.ballX += this.dx;
    this.ballY += this.dy;
    this.renderer.setStyle(this.ball.nativeElement, 'transform', `translate(${this.ballX}px, ${this.ballY}px)`);

    // Get the position and dimensions of the ball and paddle
    const ballSize = this.ball.nativeElement.getBoundingClientRect();
    let ballRadius = ballSize.width / 2;

    if (ballSize.bottom >= this.board.height) {
      this.removeBall();
    } else {
      if (ballSize.right + ballRadius >= this.board.width || ballSize.left <= 0) {
        this.dx = -this.dx;
      }
      if (ballSize.top <= 0) {
        this.dy = -this.dy;
      ...
      this.ballPaddleCollusion(ballSize);
      this.ballBricksCollusion(ballSize);
  ...
  //todo: change
  ballPaddleCollusion(ballSize: DOMRect): void {
    ...
    if (
      ballSize.left >= paddle.left &&
      ballSize.right <= paddle.right &&
      ballSize.bottom >= paddle.top
    ) {
      let ballHitPosition = ballSize.left + ballSize.width / 2 - paddle.left;
      ...
      ballSize.bottom >= paddle.top &&
      ballSize.left - ballSize.width / 2 < paddle.right &&
      ballSize.right + ballSize.width / 2 > paddle.left
    ) {
      this.dy = -this.dy;
      this.dx = -this.dx;
    }
